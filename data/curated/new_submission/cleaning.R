...
repairs <- readxl::read_xlsx(repairs_xlsx_file) %>%
  ...
  dplyr::mutate(
    ...
    if_not_repaired_why_could_you_not_repair_it_open_answer = as.character(
      if_not_repaired_why_could_you_not_repair_it_open_answer
    ),
    reparability_of_product_1_difficult_10_easy = as.integer(
      reparability_of_product_1_difficult_10_easy
    )
  )
